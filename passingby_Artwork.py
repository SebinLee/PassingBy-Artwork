...
def makeContours(frame, thresholdBoundary, listsize) :
    ...
    _, binary = cv2.threshold(grayscale,thresholdBoundary,255,cv2.THRESH_BINARY)
...
def Artwork(threshold, opacity, framerate, listsize) :
    ...
    while True :
        ...
        if ret :
            makeContours(frame,threshold,listsize)
            ...
            if(showCameraImage == True) :
                showImage = cv2.addWeighted(showImage,1,frame,opacity,0)
